"""Base Player view."""

from time import sleep
from player import Player


class PlayerView:
    """Gameset player view"""

    def add_a_new_player(self, list):
        """Add a new player in the game"""
        player_name = input("Quel est le prénom du joueur:")
        player_last_name = input("Quel est le nom du Joueur:")
        player_birth_date = input("Date de Naissance:")
        player_gender = input("Sexe (Homme, Femme, Personnel):")
        player_rank = len(list)

        new_player = Player(player_name,
                            player_last_name,
                            player_birth_date,
                            player_gender,
                            player_rank)

        list.append(new_player)
        print("Ajout du nouveau joueur '{}' terminé".format(player_name))
        sleep(2)
        return new_player.show_player()

    def show_players(self, list):
        """Present all players of tournament """
        print("Voici les {} participants du Tournoi:".format(len(list)))
        for elt in list:
            print(elt)

    def show_list_players(self, list):
        """Present all players of list """
        print("Voici les {} Joueurs disponible:".format(len(list)))
        for elt in list:
            print(elt)

        if len(list) == 0:
            print("Il n'y a aucun joueur disponible")
